...
namespace ttaenc
{
    ...
    public class AlbumMaker
    {
        ...
        private void WriteHtml(CancellationToken cancellationToken, AlbumCollectionMeta meta)
        {
            ...
            {
                meta.HtmlMediaFiles.Add(i.Name);
            }
...
Style: ");
                ...
                foreach (var album in meta.AlbumCollection.Album)
                {
                    ...
                    foreach (var i in album.Tracks)
                    {
                        ...
                        w.WriteLine("</a>");
                        w.WriteLine("<span class={0}>{1}</span> ", "track-number".Quote(), T(i.TrackNumber.ToString()));
                        w.WriteLine("<span class={0}>{1}</span> ", "track-title".Quote(), T(i.Title));
                        w.WriteLine("<span class={0}>({1})</span> ", "track-duration".Quote(), T(i.Duration.ToString(@"mm\:ss")));
                        w.WriteLine("</li>");
                    }
                    w.WriteLine("</ul>");
                    w.WriteLine(@"</div>");
